...
class Board:
    ...
    def draw(self):
        ...
        for col in range(9):
            ...
            for row in range(9):
                t.penup()
                t.goto(col * size + size / 3, -row * size + size / 3)
                if data[row][col] == 0:
                    pass
                else:
                    t.write(data[row][col], font=("Arial", 25, "normal"))
                t.goto(size * col, -size * row)
                t.pendown()
                for side in range(4):
                    t.fd(size)
                    t.left(90)
        ...
        for row in range(9):
            for col in range(9):
                if self.__data.get_value(row, col) == '0':
                    new_cell = Cell("0", col, row, self.__data.get_col(col), self.__data.get_row(row),
                                    self.__data.get_sqr(col, row))
                    new_cell.set_possibilities()
                    self.__cells.append(new_cell)
                else:
                    new_cell = Cell(str(self.__data.get_value(row, col)), col, row, self.__data.get_col(col),
                                    self.__data.get_row(row), self.__data.get_sqr(col, row))
                    self.__cells.append(new_cell)
